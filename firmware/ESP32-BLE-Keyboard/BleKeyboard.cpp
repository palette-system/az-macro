...
BleKeyboard::BleKeyboard(std::string deviceName, std::string deviceManufacturer, uint8_t batteryLevel, uint8_t kl) : hid(0)
{
...
//	}
	...
	if (_keyReport.keys[0] != kk && _keyReport.keys[1] != kk &&
		...
		_keyReport.keys[4] != kk && _keyReport.keys[5] != kk) {

		for (i=0; i<6; i++) {
			if (_keyReport.keys[i] == 0x00) {
				_keyReport.keys[i] = kk;
				break;
			}
		}
		if (i == 6) {
...
size_t BleKeyboard::press_set(uint8_t k)
{
	uint8_t i;
	unsigned short kk;
	kk = code_convert(_asciimap[k]);
	...
	_keyReport.keys[3] = 0x00;
	_keyReport.keys[4] = 0x00;
	_keyReport.keys[5] = 0x00;

	sendReport(&_keyReport);
	return 1;
}

unsigned short BleKeyboard::code_convert(unsigned short k)
{
	short i = 0;
	if (this->keyboard_language == 1) {
		while (_codemap_us[i] && i < 63) {
			if (_codemap_us[i] == k) {
				return _codemap_us[i + 1];
			}
			i += 2;
		}
	}
...
size_t BleKeyboard::press_raw(unsigned short k)
{
	uint8_t i;
	unsigned short kk;
	kk = code_convert(k);
...
size_t BleKeyboard::release_raw(unsigned short k)
{
	uint8_t i;
	unsigned short kk;
	kk = code_convert(k);
...
void BleKeyboard::set_keyboard_language(uint8_t kl)
{
  this->keyboard_language = kl;
}
